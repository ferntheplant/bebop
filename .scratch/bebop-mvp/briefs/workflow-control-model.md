...
Source tickets: [What is a safe point to interrupt a seat, and what does takeover do at each stage?](../issues/what-is-a-safe-point-to-interrupt-a-seat.md),
[What is the `sf` command surface, in use?](../issues/the-sf-command-surface.md),
[What does the cockpit look like, and where do log panes come from?](../issues/cockpit-layout-and-log-panes.md),
[What are the default constraints, and what happens when one is exhausted?](../issues/default-constraints-and-exhaustion.md),
[How is the OpenCode pin enforced, and what qualifies an upgrade?](../issues/opencode-version-pin-and-upgrade-qualification.md).

## Why now

The transition core is pure and tested through its own interface, so all of this is reachable without exe.dev,
GitHub, or a paid model. It is also the floor every later ticket lands on: the finding-severities ticket,
the evidence-surfacing ticket, and the manifest-recovery ticket all describe states this model cannot
...
[the constraints ticket's answer](../issues/default-constraints-and-exhaustion.md) first — it is the
...
- Contracts: `ConstraintProfile` takes the shape the constraints ticket settled — `validatedCandidatesPerSpec`, and a
...
1. **`reopen-spec` may land in a suspended stage.** ADR 0038 and
   ...
   the constraints ticket makes `reopen-spec` the only way to earn a fresh validated-candidate allowance: the allowance is
...
3. **Where the frozen profile lives.** Still open, and still fog on [the map](../map.md) under
...
exe.dev provisioning is still [Provision exe.dev access](../issues/provision-exe-dev-access.md) — so there is no deployed peer to stay compatible with, and a v2 that
