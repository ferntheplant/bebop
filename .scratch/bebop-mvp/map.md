...
- [The local system harness is maintained](./../local-system-harness/brief.md) — the packed-process floor now lives
  ...
  create with bootstrap identity, Swordfish-before-listener registration, API restart, daemon `SIGKILL`, local
  `sf cancel` (now operator-credential authenticated), and an offline Bebop stop all pass repeatedly over
  disposable Postgres. The machine credential travels from Bebop derivation through `LifecycleProvider.provision`
  into a one-shot bootstrap artifact the supervisor consumes, never through an operator retrieval route. The
  `bebop` CLI gained `bounty stop`. This is the floor the OpenCode driver now consumes.
