// The durable command queue (`docs/design/SYSTEM.md` §18.1, 18.4).
//
...
// both sides, so redelivering after an uncertain disconnect is safe — `docs/design/SYSTEM.md` §18.3 makes
 ...
 * back on `command_result` (`docs/design/SYSTEM.md` §18.1). Keeping delivery separate from the reported
...
export interface CommandRepositoryService {
   ...
   * takeover or a second stop (Milestone 5: "duplicate commands do not repeat takeover,
