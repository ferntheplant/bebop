// Bebop's Postgres schema (`docs/design/SYSTEM.md` §22.1).
//
...
const initial = Effect.gen(function* () {
  ...
  // Bounty identity and the lifecycle state Bebop is authoritative for (`docs/design/SYSTEM.md` §9.1).
  //
  // `status` is deliberately absent: the compact status in `docs/design/SYSTEM.md` §17.5 is derived from
  ...
  // is created and injected at its bootstrap (`docs/design/SYSTEM.md` §18.2). A bounty that has a record
  ...
  // The client-visible event log behind `GET /api/bounties/:id/events` (`docs/design/SYSTEM.md` §17.2).
  ...
  // The durable projection (`docs/design/SYSTEM.md` §9.3). `snapshot` carries the workflow core; the
  ...
  // The durable command queue (`docs/design/SYSTEM.md` §18.1). A command survives Swordfish being
  ...
  // SHA-pinned privileged-path approvals (`docs/design/SYSTEM.md` §13.3).
  yield* sql`
  ...
  // Idempotency keys (`docs/design/SYSTEM.md` §22.1). `request_fingerprint` is what makes a reused key
  ...
  // Durable background work for `bebop-worker` (`docs/design/SYSTEM.md` §25: "durable job rows in
