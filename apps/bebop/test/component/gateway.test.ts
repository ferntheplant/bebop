// The Swordfish connection gateway over a real WebSocket ("Swordfish connects outbound only" (ADR 0013)).
...
suite("Swordfish gateway", () => {
  ...
  test("delivers a command queued before the connection existed", async () => {
    ...
    // Queued while nothing is connected — `docs/capabilities/13-recovery-and-reliability.md`'s offline case.
  ...
  test("marks a connection stale when heartbeats stop, and disconnected across a restart", async () => {
    ...
    // break "Bebop owns authority, Swordfish owns the loop" (ADR 0002)'s rule outright.
