...
reconciled separately. Takeover changes the workflow controller and active seat lease, not stage; see
[One controller drives one active cowboy (ADR 0037)](./0037-one-controller-drives-one-active-cowboy.md).

## Consequences

`--force` skips the graceful period rather than weakening the no-concurrent-writers guarantee. A failed forced
restart leaves the workflow under human control and degraded instead of returning uncertain authority to
Swordfish. Handoff reverses the ordering: human access is disabled and any issued credential is revoked before
control returns, then Swordfish starts fresh work for the unchanged stage rather than resuming an aborted turn.
