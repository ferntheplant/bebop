...
validators itself, from a worktree pinned to the exact candidate commit — not from the directory the agent has
...
- **Checks ein ran itself are never authoritative.** They were useful while developing; they prove nothing about
  the commit. An agent verifying its own working directory can pass on files it never committed, ports it left
...
- [Verification runs in a clean-room worktree (ADR 0015)](../adr/0015-verification-runs-in-a-clean-room-worktree.md)
  — the clean room is what makes a gate result a statement about the commit rather than about the agent's desk,
