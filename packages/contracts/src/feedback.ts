...
// and Milestones 6 and 7 would have grown app-local types in Swordfish — exactly the drift
// the shared contracts exist to prevent.
//
 ...
 *
 * Timeout is its own case rather than an exit code, because Milestone 6 must kill the whole
 * process group on a timeout and consume that stage's attempt budget — behaviour that
