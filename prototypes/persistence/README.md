# Prototype: Postgres and SQLite persistence on the pinned Bun
...
exist, which is exactly the sort of thing this prototype is for. `SqlClient.SafeIntegers` exists as a reference if
...
`results.json` and the `.prototype/` scratch directory are gitignored. Postgres runs on a Docker-allocated host port
