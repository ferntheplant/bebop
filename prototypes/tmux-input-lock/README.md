# Prototype: tmux pane input lock
...
The prototype uses tmux to test tmux. A second tmux server on its own socket runs `tmux attach` against the seat
...
developer's tmux configuration nor their login shell can change the result (see `docs/testing.md` on fixtures).
...
```bash
vp run @bebop/prototype-tmux-input-lock#prototype
```

Exits nonzero if any probe fails. Writes `results.json` next to this file; it and the `.prototype/` scratch
...
This is a prototype, not product code. The real cockpit lands in `apps/swordfish` during Milestone 8.
