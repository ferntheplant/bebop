# Spike: Effect HTTP, SSE, WebSocket, and CLI on the pinned Bun
...
**Assumption under test:** [`SPEC.md`](../../SPEC.md) §25 — Effect 4 `effect/unstable/httpapi` for the HTTP API
...
selected Bun version." Postgres and SQLite are covered by [`spikes/persistence`](../persistence/README.md); this
spike covers the rest of that sentence.
...
One caveat this spike does **not** answer: it reads each stream to completion over a bounded set of live events.
...
worked against the same server (W2) — which is the shape of SPEC §18.1's reconnecting channel, though not yet its
reconnect _policy_.
...
rejected promise: `All fibers interrupted without error`. A spike or test that treats a nonzero exit as "probes
...
- The **real** `BebopHttpApi` is not served — that means implementing every Milestone 3 handler. The spike serves
...
- Reconnect _policy_ (backoff, resume from an acknowledged cursor) is Milestone 5's, not this spike's. W2 shows a
...
```bash
vp run @bebop/spike-effect-runtime#spike
```
...
This is a spike, not product code. The real API lands in `apps/bebop` during Milestone 3, the daemon and `sf` in
