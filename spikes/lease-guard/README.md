# Spike: OpenCode plugin control-lease guard
...
**Assumption under test:** [`SPEC.md`](../../SPEC.md) §11.5 — "the bebop plugin rejects prompt submission on a
leased seat regardless of how the prompt arrived — including a user who opens a second OpenCode client from a
shell pane."

**PLAN reference:** Milestone 0, "Prove the OpenCode plugin API can reject prompt submission for a leased seat."
...
### 2. `POST /session/:id/shell` bypasses the plugin entirely (new — not in SPEC)
...
This is a real hole in SPEC §11.5's "regardless of how the prompt arrived." A user with HTTP access to the
...
SPEC §11.5 says the plugin "tells the user to run the takeover command." That cannot be delivered through the
...
1. **SPEC §11.5's third layer is cheap, not architectural.** Two environment variables set by Swordfish when it
...
3. **OS user separation is not worth its cost here.** It is the only thing that makes bypass genuinely
   impossible, but it contradicts SPEC §16.1's free shell panes, and an operator with sudo reopens it anyway.
...
4. **The seat password is a capability, not configuration.** Bind its lifetime to the control lease: never
   ...
   `OPENCODE_DB`, but seat panes and the event subscription must be re-established. SPEC §11.5 records this.
...
Requires `opencode` 1.18.5 on `PATH`. No model credentials — the spike serves its own OpenAI-compatible
endpoint and fails loudly if anything else is contacted.

```bash
vp run @bebop/spike-lease-guard#spike
```
...
This is a spike, not product code. The real guard lands in `apps/bebop-opencode-plugin` during Milestone 7.
