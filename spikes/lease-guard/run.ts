...
const spikeDirectory = `${project}/.spike`;
const leaseFile = `${spikeDirectory}/lease.json`;
const auditFile = `${spikeDirectory}/audit.jsonl`;
const sentinel = `${spikeDirectory}/shell-ran.sentinel`;
...
const probes: Array<Probe> = [
  ...
  },
  {
    ...
    body: { model, agent: "build", command: `touch ${spikeDirectory}/shell-ran.sentinel` },
  },
];
...
try {
  await Bun.write(`${spikeDirectory}/.keep`, "", { createPath: true });
