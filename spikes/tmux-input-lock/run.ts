...
//
// SPEC section 16.2 lists the tmux input lock as the UX layer of steering safety. The
...
// change the result (PLAN section 8).
...
// The free shell pane stands in for SPEC section 16.1's shell panes. `/bin/sh` is named
// explicitly so the probe does not inherit the developer's login shell or its startup
// files (PLAN section 8).
...
try {
  ...
  await probe("T11", "any pane in the session can clear the lock with select-pane -e", async () => {
    ...
    return {
      // Passing means the spike confirmed the bypass exists. SPEC section 16.2 already
